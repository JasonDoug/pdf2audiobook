...
class JobService:
    ...
    def create_job(self, user_id: int, job_data: JobCreate, pdf_s3_key: str, pdf_s3_url: str) -> Job:
        job = Job(
            user_id=user_id,
    ...
    def update_job_status(self, job_id: int, status: JobStatus, progress: int = None, error_message: str = None):
        job = self.db.query(Job).filter(Job.id == job_id).first()
        if not job:
            return None
    ...
    def can_user_create_job(self, user_id: int) -> bool:
        ...
        if user.subscription_tier.value == "free":
            ...
            monthly_jobs = self.db.query(Job).filter(
                Job.user_id == user_id,
